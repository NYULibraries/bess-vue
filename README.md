...
* `bess-vue` is not styled using SFC `<style>` blocks because the original
widget (BobCat/Primo search embed ) was in theory supposed to be a general purpose
UI component that anyone could use, and so the "BobCat styles" had to be optional.
Several external stylesheets were created in different places and used in NYU
Libraries web pages and the web pages made by other parties.  For more technical
and historical information, see:
    * [Do not put `<style>` tags in SFCs](#do-not-put-style-tags-in-sfcs)
    * [Old stylesheet retired](#old-stylesheet-retired) in [Project history ](#project-history)
* We have decided that we will not be developing and maintaining this new
version of `bess-vue` as a general purpose widget.  We are in the process of
defining our style management policy and strategy in this work-in-progress
spreadsheet:
[BESS instance management worksheets](https://docs.google.com/spreadsheets/d/1zUcxRn5K5lVmeXgnSDRfQa_uVMvtiP04KZUQO0IKMmE/edit?gid=1782242497#gid=1782242497).
Refer to this document to learn more about page owners and permissions,
style management and implementation, and other related matters.  Certain
capitalized terms used in this README are defined in that spreadsheet.
* There are currently two `bess-vue` stylesheets:
    * _public/bess.min.css_: this could be considered the "default" stylesheet, although
in some respects "default" is a misnomer.
    * _public/bess-dol.min.css_: this is a transition stylesheet for the DoL Home Page
Host Page.  The current _bess.min.css_ code unfortunately did not work as a drop-in
replacement for the widget CSS already defined in the DoL Home Page, so a dedicated
stylesheet was needed.
        * The initial version of this stylesheet was made by copying a contiguous
block of `bess-vue` (BobCat/Primo search widget) CSS from
https://library.nyu.edu/assets/css/global.css?cache_bust=20240403 on February 26, 2026.
According to the CircleCI records, this CSS was generated by
[library.nyu.edu/_sass /_search.scss](https://github.com/NYULibraries/library.nyu.edu/blob/2648a700aa79adb4b8c8113cca389c1cd200112a/_sass/_search.scss#L66-L297).
  * Ideally, we will eventually consolidate these stylesheets into a single
file to be used with all Host Pages in the DoL Style Group.
